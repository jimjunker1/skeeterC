...
<!-- # transform and sample from Poisson with posterior draws -->
<!-- pois_glmm_pred = map_dbl(unlist(pois_glmm_draws$b_Intercept),unlist(pois_glmm_draws$b_litter_days), ~sum(rpois(27,exp(.x+.y)))) -->
...
<!--   geom_point(data = poisSLMM_accDf[(lastVal+1),], aes(x = date, y = prediction), size = 2, color = 'grey')+ -->
<!--   geom_line(aes(x = date, y = count), linewidth = 1.1, color = 'black')+ -->
<!--   geom_point(aes(x = date, y = count), size = 1.2, color = 'black')+ -->
<!--   scale_y_continuous(name = "Skeeter Rafts (#)", limits = ylims)+ -->
<!--   scale_x_continuous(breaks = labelsDf$date, labels = labelsDf$datelab, sec.axis = dup_axis(labels = labelsDf$day))+ -->
...
<!-- # transform and sample from Poisson with posterior draws -->
<!-- nb_glmm_pred = map2_dbl(unlist(nb_glmm_draws$b_Intercept), unlist(nb_glmm_draws$shape), ~sum(rnbinom(27,mu = exp(.x), size = .y))) -->
...
<!-- nbGLMMMedian = nb_glmm_pred %>% median -->

<!-- nbGLMMInt = nb_glmm_pred %>% quantile(c(0.025, 0.975)) -->
...
<!-- gamm_pred = brm(count ~ s(litter_days, bs = 'tp', k = 5) + s(block, bs = 're', k = 5),  -->
<!--                 data = exp_df, -->
<!--                 family = poisson(), -->
<!--                 iter = 1000, -->
<!--                 thin = 1, -->
<!--                 file = here::here("data/models/glmm_pred"), -->
<!--                 file_refit = "on_change", -->
<!--                 chain = 4, -->
<!--                 cores = 4) -->

<!-- y = exp_df %>% mutate(prediction = predict(gamm_pred, newdata = exp_df[,c("litter_days","block")])[1], type = "response") -->
